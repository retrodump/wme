...
namespace Wme
{
	class WmeDllExport KeyboardEvent
	{
	public:
		enum EventType
		{
		...
		// we use the same codes as OIS for simplicity
		enum KeyCode
		{
			KEY_UNASSIGNED  = 0x00,
			KEY_ESCAPE      = 0x01,
			...
			KEY_PREVTRACK   = 0x90,    // Previous Track (KC_CIRCUMFLEX on Japanese keyboard)
		...
		KeyboardEvent(EventType type, KeyCode keyCode, const WideString& text);
		KeyboardEvent(EventType type, OIS::Keyboard* oisKeyboard, const OIS::KeyEvent& oisEvent);
		~KeyboardEvent();
		...
		void FromOis(OIS::Keyboard* oisKeyboard, const OIS::KeyEvent& oisEvent);
